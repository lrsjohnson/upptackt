...
;;; (3) Mark the second component as extending from the first at that merged
;;; point-connection via some arbitrary angle

;;; (4) For each new pair of points, check if their distance has been
;;; established in any other diagram, add "constraints"
;;; or try to rotate the new angle to satisfy such

;;; Rotate + extend one length? [extend length for desired distance, then rotate
;;; to meet desired point]
